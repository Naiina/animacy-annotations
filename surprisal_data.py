...
def get_json_labels(UD_file,out_folder,max_len = -1):
    """
    Input: UD file with anim annot, output file name
    Outputs: csv file and json dict  with 
                    sent id, 
    ...
    l = len(l_sent_id)
    l_idx = [i for i in range(l)]
    random.shuffle(l_idx)

    s1 = int(0.8 * len(l_idx))
    s2 = int(0.1 * len(l_idx))

    l_idx_train = l_idx[:s1]
    l_idx_val = l_idx[s1 + s2:]
    l_idx_test = l_idx[s1:s1 + s2]

    d_train = {"sent_id": [l_sent_id[i] for i in l_idx_train], "sentences": [ll_sent[i] for i in l_idx_train],"anim_id":[ll_anim_id[i] for i in l_idx_train],"anim_labels" : [ll_anim_labels[i] for i in l_idx_train]} 
    d_val = {"sent_id": [l_sent_id[i] for i in l_idx_val], "sentences": [ll_sent[i] for i in l_idx_val],"anim_id":[ll_anim_id[i] for i in l_idx_val],"anim_labels" : [ll_anim_labels[i] for i in l_idx_val]} 
    d_test = {"sent_id": [l_sent_id[i] for i in l_idx_test], "sentences": [ll_sent[i] for i in l_idx_test],"anim_id":[ll_anim_id[i] for i in l_idx_test],"anim_labels" : [ll_anim_labels[i] for i in l_idx_test]} 

    
    
    df = pd.DataFrame.from_dict(d_train)
    df.to_csv(out_folder+'/csv/train_surprisal_text_labels_'+UD_file[19:21]+".csv", index=False)
    with open(out_folder+'/json/train_surprisal_text_labels_'+UD_file[19:21]+".json", "w",encoding="utf-8") as json_file:
        json.dump(d, json_file,indent=4, ensure_ascii=False)
    
    df = pd.DataFrame.from_dict(d_val)
    df.to_csv(out_folder+'/csv/val_surprisal_text_labels_'+UD_file[19:21]+".csv", index=False)
    with open(out_folder+'/json/val_surprisal_text_labels_'+UD_file[19:21]+".json", "w",encoding="utf-8") as json_file:
        json.dump(d, json_file,indent=4, ensure_ascii=False)
    
    df = pd.DataFrame.from_dict(d_test)
    df.to_csv(out_folder+'/csv/test_surprisal_text_labels_'+UD_file[19:21]+".csv", index=False)
    with open(out_folder+'/json/test_surprisal_text_labels_'+UD_file[19:21]+".json", "w",encoding="utf-8") as json_file:
        json.dump(d, json_file,indent=4, ensure_ascii=False)
...
if __name__ == "__main__":
    ...
    out_folder = "full_datasets"
    for file in os.listdir(UD_dir):
        UD_file = "UD_with_anim_annot/"+file
        get_json_labels(UD_file,out_folder,max_len = -1)
